...
def BuildMQuery(content, vchtime, counterIds, instance, vm, interval):
    perfManager = content.perfManager
    metricId = vim.PerformanceManager.MetricId(counterId=counterId, instance=instance)
    startTime = vchtime - timedelta(minutes=(interval + 1))
    endTime = vchtime - timedelta(minutes=1)
...
def html_table(vm_property, vm_value):
    ...
    print(vm_property +" => "+str(vm_value))
    # print('</tr>')

def PrintHostInfo(host, content, vchtime, interval, perf_dict):
    ...
    print(host.hardware)
    print("H3a")
    print(vim.host.SystemIdentificationInfo())
    print("H3b")

    print ("H4")
    print(host.capability)
    print ("H5")
    print(host.licensableResource)
    print ("H6")
    ...
    print(host.vm)
    print ("H9")
    print(host.datastore)
    print ("Ha")
    print(host.network)
    print ("Hb")
    print(host.systemResources)
    print("uptime: "+str(host.RetrieveHardwareUptime()))
    statCpuReady = BuildQuery(content, vchtime, (Perf(perf_dict, 'cpu.ready.summation')), "", host, interval)
    cpuReady = (float(sum(statCpuReady[0].value[0].value)) / statInt)
    print (statCpuReady)
    print ("Hc")
    print (cpuReady)
    print("Hd")
    statCpuUsage = BuildQuery(content, vchtime, (StatCheck(perf_dict, 'cpu.usage.average')), "", host, interval)
    print(statCpuUsage)
    print("He")
    cpuUsage = ((float(sum(statCpuUsage[0].value[0].value)) / statInt) / 100)
    print(cpuUsage)
    print("Hf")
    cpu=BuildQuery(content, vchtime, (StatCheck(perf_dict, 'cpu.ready.summation')), "*", host, interval)
    print (cpu)
    print("Hg")
    print(BuildQuery(content, vchtime, (StatCheck(perf_dict, 'datastore.numberReadAveraged.average')),
                                     "*", host, interval))
...
def GetProperties(content, reqType, props):
    viewType=[reqType]
    specType=reqType
...
#            return -1
        ...
        print(perf_dict)
        print("Pe")
        retProps = GetProperties(content, vim.VirtualMachine, ['name', 'runtime.powerState'])
        ...
        retProps=GetProperties(content,vim.HostSystem,['name','runtime.powerState'])
        for host in retProps:
            print ("P1")
            print (host)
            print ("P2")
            print (content.perfManager.QueryAvailablePerfMetric(host['moref']))
            print ("P3")
            PrintHostInfo(host['moref'], content, vchtime, int(form['vminterval']), perf_dict)
